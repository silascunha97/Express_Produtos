(function(global, factory) {
	typeof exports === "object" && typeof module !== "undefined" ? factory(exports) : typeof define === "function" && define.amd ? define(["exports"], factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, factory(global.apidomReference = {}));
	...
	*/
	...
	//#endregion
	//#region ../../node_modules/ramda/es/internal/_dropLastWhile.js
	...
	//#endregion
	//#region ../../node_modules/ramda/es/internal/_xdropLastWhile.js
	var XDropLastWhile = /*#__PURE__*/ function() {
		...
		XDropLastWhile.prototype["@@transducer/init"] = _xfBase_default.init;
	...
	//#endregion
	//#region ../../node_modules/ramda/es/dropLastWhile.js
	/**
	...
	*/
	...
	//#endregion
	//#region ../../node_modules/ramda/es/lens.js
	/**
	...
	*/
	...
	//#endregion
	//#region ../../node_modules/ramda/es/over.js
	...
	*/
	...
	//#endregion
	//#region ../../node_modules/ramda/es/propEq.js
	/**
	...
	*/
	...
	//#endregion
	//#region ../../node_modules/ramda/es/when.js
	/**
	...
	*/
	...
	//#endregion
	//#region ../../node_modules/ramda-adjunct/es/isRegExp.js
	/**
	...
	*/
	...
	//#endregion
	//#region ../../node_modules/ramda-adjunct/es/escapeRegExp.js
	/**
	...
	*/
	...
	//#endregion
	//#region ../../node_modules/ramda-adjunct/es/internal/ponyfills/String.replaceAll.js
	var checkArguments = function checkArguments(searchValue, replaceValue, str) {
		if (str == null || searchValue == null || replaceValue == null) throw TypeError("Input values must not be `null` or `undefined`");
	};
	var checkValue = function checkValue(value, valueName) {
		if (typeof value !== "string") {
			if (!(value instanceof String)) throw TypeError("`".concat(valueName, "` must be a string"));
		}
	};
	var checkSearchValue = function checkSearchValue(searchValue) {
		if (typeof searchValue !== "string" && !(searchValue instanceof String) && !(searchValue instanceof RegExp)) throw TypeError("`searchValue` must be a string or an regexp");
	};
	//#endregion
	//#region ../../node_modules/ramda-adjunct/es/replaceAll.js
	var replaceAllPonyfill = curryN(3, function replaceAll(searchValue, replaceValue, str) {
		checkArguments(searchValue, replaceValue, str);
		checkValue(str, "str");
		checkValue(replaceValue, "replaceValue");
		checkSearchValue(searchValue);
		return replace(new RegExp(isRegExp(searchValue) ? searchValue : escapeRegExp(searchValue), "g"), replaceValue, str);
	});
	...
	*/
	...
	//#endregion
	//#region ../../node_modules/ramda-adjunct/es/trimCharsEnd.js
	/**
	...
	*/
	...
	var isWindows = () => /^win/.test(globalThis.process?.platform ?? "");
	/**
	...
	*/
	var getProtocol = (url) => {
		try {
			return trimCharsEnd(":", new URL(url).protocol);
		} catch {
			return;
		}
	};
	...
	* @public
	*/
	var hasProtocol = pipe(getProtocol, isNotUndefined);
	/**
	...
	*/
	var getExtension = (url) => {
		...
		if (lastDotPosition >= 0) return url.substring(lastDotPosition).toLowerCase();
		return "";
	};
	/**
 ...
 /**
	...
	*/
	var toFileSystemPath = (uri, options) => {
		...
		if (isFileUrl) {
			...
			if (isWindowsPredicate() && path[1] === "/") path = `${path[0]}:${path.substring(1)}`;
		...
		if (isWindowsPredicate() && !isFileUrl) {
			...
			if (path.substring(1, 3) === ":\\") path = path[0].toUpperCase() + path.substring(1);
		}
		return path;
	};
	/**
	...
	*/
	var fromFileSystemPath = (uri) => {
		...
		path = encodeURI(path);
		for (let i = 0; i < urlEncodePatterns.length; i += 2) path = path.replace(urlEncodePatterns[i], urlEncodePatterns[i + 1]);
		return path;
	};
	/**
	...
	*/
	var getHash = (uri) => {
		...
		if (hashIndex !== -1) return uri.substring(hashIndex);
		return "#";
	};
	/**
	...
	*/
	var stripHash = (uri) => {
		...
		if (hashIndex >= 0) hashStrippedUri = uri.substring(0, hashIndex);
	...
	*/
	var cwd = () => {
		if (globalThis.window !== void 0) return stripHash(globalThis.location.href);
		const path = globalThis.process?.cwd() ?? "/";
		const lastChar = last(path);
		if (["/", "\\"].includes(lastChar)) return path;
	...
	*/
	var sanitize = (uri) => {
		if (isFileSystemPath(uri)) return fromFileSystemPath(toFileSystemPath(uri));
		try {
	...
	*/
	var unsanitize = (uri) => {
		if (isFileSystemPath(uri)) return toFileSystemPath(uri);
	...
	*/
	var File = class {
		...
		get extension() {
			if (isString(this.uri)) return getExtension(this.uri);
			return "";
		}
		toString() {
			if (typeof this.data === "string") return this.data;
			if (this.data instanceof ArrayBuffer || ["ArrayBuffer"].includes(type(this.data)) || ArrayBuffer.isView(this.data)) return new TextDecoder("utf-8").decode(this.data);
			return String(this.data);
		}
	};
	//#endregion
	//#region src/ReferenceSet.ts
	/**
	* @public
	*/
	var ReferenceSet = class {
		...
		merge(anotherRefSet) {
			for (const reference of anotherRefSet.values()) this.add(reference);
			return this;
		}
		has(thing) {
	...
	*/
	...
	//#endregion
	//#region src/options/util.ts
	/**
	* Algorithm for deep merging options.
	*/
	var baseURILens = lens(path(["resolve", "baseURI"]), assocPath(["resolve", "baseURI"]));
	var baseURIDefault = (baseURI) => isEmptyString(baseURI) ? cwd() : baseURI;
	/**
	* @public
	*/
	var merge = (lObj, rObj) => {
		return over(baseURILens, baseURIDefault, mergeDeepRight(lObj, rObj));
	};
	//#endregion
	...
	*/
	...
	//#endregion
	//#region src/errors/PluginError.ts
	/**
	* @public
	*/
	var PluginError = class extends ApiDOMError {
		plugin;
		constructor(message, options) {
	...
	*/
	var filter = async (method, parameters, plugins) => {
	...
	*/
	var run = async (method, parameters, plugins) => {
		...
		for (const plugin of plugins) try {
			return {
				plugin,
				result: await plugin[method].call(plugin, ...parameters)
			};
		} catch (error) {
	...
	* @public
	*/
	var ParseError = class extends ApiDOMError {};
	//#endregion
	//#region src/errors/ResolveError.ts
	/**
	* @public
	*/
	var ResolveError = class extends ApiDOMError {};
	//#endregion
	//#region src/errors/ResolverError.ts
	/**
	* @public
	*/
	var ResolverError = class extends ResolveError {};
	//#endregion
	//#region src/errors/UnmatchedResolverError.ts
	/**
	* @public
	*/
	var UnmatchedResolverError = class extends ResolverError {};
	//#endregion
	//#region src/resolve/util.ts
	var CACHE_NAME = "apidom-file-cache";
	var getCacheFileResult = async ({ cacheKey, cacheTTL }) => {
		...
		try {
			...
			if (response) {
				...
				if (Date.now() - timestamp < cacheTTL) return {
					cachedResult,
					cachedError
				};
	...
	var setCacheFileResult = async ({ cacheKey, result, error, cacheTTL }) => {
		if (cacheTTL === 0) return;
		try {
			const cacheData = {
				cachedResult: result,
			...
			const cache = await caches.open(CACHE_NAME);
			const response = new Response(JSON.stringify(cacheData), { headers: { "Content-Type": "application/json" } });
	...
	*/
	var readFile$1 = async (file, options) => {
		...
		if (cachedError !== null) throw new ResolveError(`Error while reading file "${file.uri}"`, { cause: cachedError });
		const optsBoundResolvers = options.resolve.resolvers.map((resolver) => {
			return Object.assign(Object.create(resolver), options.resolve.resolverOpts);
		});
		const resolvers = await filter("canRead", [file, options], optsBoundResolvers);
		if (isEmpty(resolvers)) throw new UnmatchedResolverError(file.uri);
		try {
			const { result } = await run("read", [file], resolvers);
	...
	*/
	var parseFile = async (file, options) => {
		const optsBoundParsers = options.parse.parsers.map((parser) => {
			return Object.assign(Object.create(parser), options.parse.parserOpts);
		});
		const parsers = await filter("canParse", [file, options], optsBoundParsers);
		if (isEmpty(parsers)) throw new UnmatchedResolverError(file.uri);
		try {
			const { plugin, result } = await run("parse", [file, options], parsers);
			if (!plugin.allowEmpty && result.isEmpty) return Promise.reject(new ParseError(`Error while parsing file "${file.uri}". File is empty.`));
			return result;
		} catch (error) {
	...
	*/
	var parse$1 = async (uri, options) => {
		...
		*/
		...
		const data = await readFile$1(file, options);
	...
	* @public
	*/
	var getNodeType$1 = (node) => node === null || node === void 0 ? void 0 : node.type;
	/**
	* @public
	*/
	var isNode$1 = (node) => typeof getNodeType$1(node) === "string";
	/**
	* @public
	*/
	...
	* @public
	*/
	var mergeAll = (visitors, { visitFnGetter = getVisitFn, nodeTypeGetter = getNodeType$1, breakSymbol = BREAK, deleteNodeSymbol = null, skipVisitingNodeSymbol = false, exposeEdits = false } = {}) => {
	...
	var mergeAllAsync = (visitors, { visitFnGetter = getVisitFn, nodeTypeGetter = getNodeType$1, breakSymbol = BREAK, deleteNodeSymbol = null, skipVisitingNodeSymbol = false, exposeEdits = false } = {}) => {
		const skipSymbol = Symbol("skip");
		const skipping = new Array(visitors.length).fill(skipSymbol);
		return {
	...
	*  @public
	*/
	var visit$1 = (root, visitor, { keyMap = null, state = {}, breakSymbol = BREAK, deleteNodeSymbol = null, skipVisitingNodeSymbol = false, visitFnGetter = getVisitFn, nodeTypeGetter = getNodeType$1, nodePredicate = isNode$1, nodeCloneFn = cloneNode$1, detectCycles = true, detectCyclesCallback = null } = {}) => {
	...
	*/
	visit$1[Symbol.for("nodejs.util.promisify.custom")] = async (root, visitor, { keyMap = null, state = {}, breakSymbol = BREAK, deleteNodeSymbol = null, skipVisitingNodeSymbol = false, visitFnGetter = getVisitFn, nodeTypeGetter = getNodeType$1, nodePredicate = isNode$1, nodeCloneFn = cloneNode$1, detectCycles = true, detectCyclesCallback = null } = {}) => {
		const visitorKeys = keyMap || {};
		let stack;
		let inArray = Array.isArray(root);
		let keys = [root];
	...
	*/
	...
	var isNode = pipe(getNodeType, isString);
	/**
	* @public
	*/
	var keyMapDefault = {
	...
	*/
	...
	visit[Symbol.for("nodejs.util.promisify.custom")] = async (root, visitor, { keyMap = keyMapDefault, ...rest } = {}) => {
	...
	*/
	...
	var refract = (value, { Type, plugins = [] }) => {
		/**
	...
	import_minim.ObjectElement.refract = createRefractor(import_minim.ObjectElement);
	import_minim.ArrayElement.refract = createRefractor(import_minim.ArrayElement);
	import_minim.StringElement.refract = createRefractor(import_minim.StringElement);
	import_minim.BooleanElement.refract = createRefractor(import_minim.BooleanElement);
	import_minim.NullElement.refract = createRefractor(import_minim.NullElement);
	import_minim.NumberElement.refract = createRefractor(import_minim.NumberElement);
	import_minim.LinkElement.refract = createRefractor(import_minim.LinkElement);
	import_minim.RefElement.refract = createRefractor(import_minim.RefElement);
	...
	//#endregion
	//#region src/errors/UnmatchedResolveStrategyError.ts
	/**
	* @public
	*/
	var UnmatchedResolveStrategyError = class extends ResolveError {};
	//#endregion
	//#region src/resolve/index.ts
	/**
	* Resolves ApiDOM with all its external references.
	*/
	var resolveApiDOM$1 = async (element, options) => {
		...
		const file = new File({
			uri: sanitize(stripHash(options.resolve.baseURI)),
			parseResult,
			mediaType: options.parse.mediaType
		});
		const resolveStrategies = await filter("canResolve", [file, options], options.resolve.strategies);
		if (isEmpty(resolveStrategies)) throw new UnmatchedResolveStrategyError(file.uri);
		try {
			const { result } = await run("resolve", [file, options], resolveStrategies);
			return result;
		} catch (error) {
	...
	*/
	var resolve$1 = async (uri, options) => {
		return resolveApiDOM$1(await parse$1(uri, options), merge(options, { resolve: { baseURI: sanitize(uri) } }));
	};
	//#endregion
	//#region src/errors/DereferenceError.ts
	/**
	* @public
	*/
	var DereferenceError = class extends ApiDOMError {};
	//#endregion
	//#region src/errors/UnmatchedDereferenceStrategyError.ts
	/**
	* @public
	*/
	var UnmatchedDereferenceStrategyError = class extends DereferenceError {};
	//#endregion
	//#region src/dereference/index.ts
	/**
	* Dereferences ApiDOM with all its external references.
	*/
	var dereferenceApiDOM$1 = async (element, options) => {
		let parseResult = element;
		...
		const dereferenceStrategies = await filter("canDereference", [file, options], options.dereference.strategies);
		if (isEmpty(dereferenceStrategies)) throw new UnmatchedDereferenceStrategyError(file.uri);
		try {
			const { result } = await run("dereference", [file, options], dereferenceStrategies);
	...
	*/
	var dereference$1 = async (uri, options) => {
		const { refSet } = options.dereference;
		const sanitizedURI = sanitize(uri);
		let parseResult;
		if (refSet !== null && refSet.has(sanitizedURI)) ({value: parseResult} = refSet.find(propEq(sanitizedURI, "uri")));
		else parseResult = await parse$1(uri, options);
		const mergedOptions = merge(options, {
			resolve: { baseURI: sanitizedURI },
			dereference: { immutable: options.dereference.immutable && refSet !== null }
		});
		return dereferenceApiDOM$1(parseResult, mergedOptions);
	};
	//#endregion
	//#region src/errors/BundleError.ts
	/**
	* @public
	*/
	var BundleError = class extends ApiDOMError {};
	//#endregion
	//#region src/errors/UnmatchedBundleStrategyError.ts
	/**
	* @public
	*/
	var UnmatchedBundleStrategyError = class extends BundleError {};
	//#endregion
	//#region src/bundle/index.ts
	/**
	* Bundle a file with all its external references to a compound document.
	*/
	var bundle$1 = async (uri, options) => {
		const { refSet } = options.bundle;
		const sanitizedURI = sanitize(uri);
		const mergedOptions = merge(options, { resolve: { baseURI: sanitizedURI } });
		let parseResult;
		if (refSet !== null && refSet.has(sanitizedURI)) ({value: parseResult} = refSet.find(propEq(sanitizedURI, "uri")));
		else parseResult = await parse$1(uri, mergedOptions);
		const file = new File({
			uri: mergedOptions.resolve.baseURI,
			parseResult,
			mediaType: mergedOptions.parse.mediaType
		});
		const bundleStrategies = await filter("canBundle", [file, mergedOptions], mergedOptions.bundle.strategies);
		if (isEmpty(bundleStrategies)) throw new UnmatchedBundleStrategyError(file.uri);
		try {
			const { result } = await run("bundle", [file, mergedOptions], bundleStrategies);
			return result;
		} catch (error) {
	...
	//#endregion
	//#region src/parse/parsers/Parser.ts
	/**
	* @public
	*/
	var Parser = class {
	...
	//#endregion
	//#region src/resolve/resolvers/Resolver.ts
	/**
	* @public
	*/
	var Resolver = class {
	...
	//#endregion
	//#region src/resolve/resolvers/HTTPResolver.ts
	/**
	* @public
	*/
	var HTTPResolver = class extends Resolver {
		timeout;
		redirects;
		withCredentials;
		constructor(options) {
			const { name = "http-resolver", timeout = 5e3, redirects = 5, withCredentials = false } = options ?? {};
			super({ name });
	...
	//#endregion
	//#region src/resolve/strategies/ResolveStrategy.ts
	/**
	* @public
	*/
	...
	//#endregion
	//#region src/dereference/strategies/DereferenceStrategy.ts
	/**
	* @public
	*/
	...
	//#endregion
	//#region src/dereference/util.ts
	/**
	* @public
	*/
	var AncestorLineage = class extends Array {
		...
		includes(searchElement, fromIndex) {
			if (searchElement instanceof Set) return super.includes(searchElement, fromIndex);
			return this.some((ancestors) => ancestors.has(searchElement));
		}
		findItem(predicate) {
			for (const set of this) for (const item of set) if (isElement(item) && predicate(item)) return item;
		}
	};
	//#endregion
	//#region src/bundle/strategies/BundleStrategy.ts
	/**
	* @public
	*/
	var BundleStrategy = class {
		name;
		constructor({ name }) {
			this.name = name;
		}
	...
	*/
	...
	//#endregion
	//#region src/errors/MaximumBundleDepthError.ts
	/**
	* @public
	*/
	var MaximumBundleDepthError = class extends BundleError {};
	//#endregion
	//#region src/errors/EvaluationElementIdError.ts
	/**
	* @public
	*/
	var EvaluationElementIdError = class extends ApiDOMError {};
	//#endregion
	//#region src/errors/JsonSchema$anchorError.ts
	/**
	* @public
	*/
	var JsonSchema$anchorError = class extends ApiDOMError {};
	//#endregion
	//#region src/errors/EvaluationJsonSchema$anchorError.ts
	/**
	* @public
	*/
	var EvaluationJsonSchema$anchorError = class extends JsonSchema$anchorError {};
	//#endregion
	//#region src/errors/JsonSchemaUriError.ts
	/**
	* @public
	*/
	var JsonSchemaUriError = class extends ApiDOMError {};
	//#endregion
	//#region src/errors/EvaluationJsonSchemaUriError.ts
	/**
	* @public
	*/
	var EvaluationJsonSchemaUriError = class extends JsonSchemaUriError {};
	//#endregion
	//#region src/errors/InvalidJsonSchema$anchorError.ts
	/**
	* @public
	*/
	var InvalidJsonSchema$anchorError = class extends JsonSchema$anchorError {
		constructor(anchor) {
			super(`Invalid JSON Schema $anchor "${anchor}".`);
		}
	};
	//#endregion
	//#region src/errors/MaximumDereferenceDepthError.ts
	/**
	* @public
	*/
	var MaximumDereferenceDepthError = class extends DereferenceError {};
	//#endregion
	//#region src/errors/MaximumResolveDepthError.ts
	/**
	* @public
	*/
	var MaximumResolveDepthError = class extends ResolveError {};
	//#endregion
	//#region src/errors/ParserError.ts
	/**
	* @public
	*/
	var ParserError = class extends ParseError {};
	//#endregion
	//#region src/index.ts
	/**
	* @public
	*/
	var readFile = async (uri, options = {}) => {
		...
		return readFile$1(new File({ uri: sanitize(uri) }), mergedOptions);
	};
	/**
	* @public
	*/
	var parse = async (uri, options = {}) => {
		return parse$1(uri, merge(defaultOptions, options));
	};
	/**
	* @public
	*/
	var resolve = async (uri, options = {}) => {
		return resolve$1(uri, merge(defaultOptions, options));
	};
	/**
	* @public
	*/
	var resolveApiDOM = async (element, options = {}) => {
		return resolveApiDOM$1(element, merge(defaultOptions, options));
	};
	/**
	* @public
	*/
	var dereference = async (uri, options = {}) => {
		return dereference$1(uri, merge(defaultOptions, options));
	};
	/**
	* @public
	*/
	var dereferenceApiDOM = async (element, options = {}) => {
		return dereferenceApiDOM$1(element, merge(defaultOptions, options));
	};
	/**
	* @public
	*/
	var bundle = async (uri, options = {}) => {
		return bundle$1(uri, merge(defaultOptions, options));
	};
	//#endregion
